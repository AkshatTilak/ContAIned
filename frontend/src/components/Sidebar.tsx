...
export const Sidebar: React.FC<SidebarProps> = () => {
  ...
  return (
    <aside
      className={`bg-[var(--bg-input)] border-r border-[var(--border-default)] flex flex-col justify-between p-3 select-none shrink-0 transition-all duration-300 ${
        sidebarCollapsed ? "w-16" : "w-[260px]"
      }`}
    >
      <div>
        {/* Brand Header & Toggle */}
        <div className="flex items-center justify-between px-1 py-2 mb-4 border-b border-[var(--border-subtle)] pb-3">
          <NavLink
            to="/system"
            className="flex items-center gap-3 overflow-hidden group"
            title="ContAIned Platform V3"
          >
            <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-emerald-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform shrink-0">
              <Server className="w-5 h-5 text-white" />
            </div>
            {!sidebarCollapsed && (
              <div className="overflow-hidden">
                <h1 className="font-bold text-white tracking-wide text-base leading-tight font-display whitespace-nowrap">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-[var(--bg-elevated)] border border-transparent hover:border-[var(--border-subtle)] transition-colors"
                  ...
                  <>
                    {isActive && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-[var(--emerald-soft)] border border-emerald-500/30 rounded-lg"
                    ...
                    {!sidebarCollapsed && (
                      <div className="flex items-center justify-between w-full relative z-10 overflow-hidden">
                        ...
                        <span className="text-[10px] font-mono text-zinc-500 px-1.5 py-0.5 bg-[var(--bg-surface)] rounded border border-[var(--border-subtle)]">
                          {item.shortcut}
                        </span>
                      </div>
                    )}
